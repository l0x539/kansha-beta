...
const OurMethodPage = () => {
  ...
  const setActiveTab = (tab: string) => {
    ...
    router.push(
      "/services/our-method?" + createQueryString("tab", `${tabIndex + 1}`),
      {
        scroll: true,
      }
    );
  ...
  const bind = useWheel(({ direction: [_, y], intentional, last }) => {
    if (last) {
  ...
  return (
    <div {...bind()}>
      <Tabs
        ...
        tab={
          tabs[parseInt(`${searchParams.get("tab") || 1}`) - 1] ?? "research"
        }
      >
        {TABS.map(({ content, label, title }, index) => (
          <Tab key={0} label={label} title={title}>
            {content}
          </Tab>
        ))}
      </Tabs>
